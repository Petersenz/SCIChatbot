...
export function FlowButton({text = 'Modern Button', subtitle, href, state = 'idle', className = '', disabled, onClick, ...props}: FlowButtonProps) {
  const blocked = disabled || state === 'loading';
  const content = <>
    <ArrowRight aria-hidden="true" className="flow-arrow flow-arrow-in" />
    <span className="flow-label"><span>{text}</span>{subtitle && <small>{subtitle}</small>}</span>
    <span aria-hidden="true" className="flow-circle" />
    {state === 'loading' ? <LoaderCircle aria-hidden="true" className="flow-status flow-spinner" /> : state === 'success' ? <Check aria-hidden="true" className="flow-status" /> : state === 'error' ? <CircleAlert aria-hidden="true" className="flow-status" /> : <ArrowRight aria-hidden="true" className="flow-arrow flow-arrow-out" />}
  </>;
